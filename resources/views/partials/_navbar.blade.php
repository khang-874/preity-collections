<div class="w-3/4 mx-auto">
    ...
    @foreach($categories as $category)
        <div class='peer/category{{$category->id}} row-start-1 bg-red-500 p-2'>
            <a href="/?category={{$category->id}}" class="block"> Category: {{$category -> name}}</a> 
        </div> 
        <div class="hidden hover:inline-flex peer-hover/category{{$category->id}}:inline-flex gap-4 row-start-2 col-span-full w-full p-2">
            ...
            <div class="" >
                <a href="/?section={{$section->id}}">Section: {{$section -> name}}</a>
                @foreach ($section->subsections as $subsection)
                    <div>
                        <a href="/?subsection={{$subsection->id}}">Subsection: {{$subsection->name}}</a> 
                    </div>
                @endforeach
            </div>
            @endforeach        
